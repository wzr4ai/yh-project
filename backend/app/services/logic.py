...
async def dashboard_report(session: AsyncSession) -> schemas.DashboardReportResponse:
    ...
    day_items = (await session.execute(sa.select(SalesItem).where(sa.func.date(SalesItem.created_at) == today))).scalars().all()
    ...
    promo_candidates = [
        item for item in sales_all["items"] if (item.get("profit_margin") or 0) <= 20 and item.get("sales_amount", 0) > 0
    ][:10]

    top_sales_today = sales_today["items"][:5]
    top_sales_all = sales_all["items"][:5]
    sales_concentration_today = (
        round2(
    ...
    report = {
        ...
        "seasonality_context": {
            ...
            "season_window": "2026年2月7日（腊月20）~2026年3月3日（正月15）",
            "peak_days": ["除夕", "大年初一"],
